...
func GetCustomerById(c echo.Context) error {
	...
	return c.JSON(http.StatusFound, allCustomers[designatedIdx])
}
...
func UpdateCustomerData(c echo.Context) error {
	...
	newData := Customer{
		Id:    allCustomers[designatedIdx].Id,
		Name:  req.Name,
		Email: req.Email,
	}

	allCustomers[designatedIdx] = newData

	log.Println("Update Customer Data Request")

	return c.JSON(http.StatusAccepted, allCustomers[designatedIdx])
}
